...
struct Storybrain {
    ...
    mutating func numbercounting() -> Int {
        return number
    }
    
    mutating func ansercheck(_ sen : String) {
        
        if sen == Qusen[number].choice1{
            number = Qusen[number].choice1Destination
        }
        
        else if sen == Qusen[number].choice2{
            number = Qusen[number].choice2Destination
        }
